...
class HashShufflingOperatorBase(PhysicalOperator, SubProgressBarMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # `DataContext.min_parallelism`). Computed before deriving
        # `target_num_partitions` below so the estimate-driven candidate can be
        # capped against it.
        ...
        #     capped to the aggregator pool size (partitions beyond that just get
        #     round-robin-assigned multiple-per-actor, costing more small
        #     submit()/finalize() hand-offs for the same total bytes moved --
        #     DSv2-only, since V1's own read-task-count-driven estimates must be
        #     left exactly as-is; see hash_shuffle_cap_target_partitions_to_aggregators)
        #   - Default configured hash-shuffle parallelism (200)
        estimated_target_num_partitions = (
            max(estimated_input_blocks) if all(estimated_input_blocks) else None
        )
        if (
            estimated_target_num_partitions is not None
            and data_context.use_datasource_v2
            and data_context.hash_shuffle_cap_target_partitions_to_aggregators
        ):
            estimated_target_num_partitions = min(
                estimated_target_num_partitions, max_shuffle_aggregators
            )

        target_num_partitions: int = (
            num_partitions
            or estimated_target_num_partitions
        ...
        # (max_shuffle_aggregators computed earlier, above)
